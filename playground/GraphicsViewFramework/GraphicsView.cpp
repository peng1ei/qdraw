...
GraphicsView::GraphicsView(QWidget *parent)
    : QGraphicsView(parent), d_ptr(new GraphicsViewPrivate) {
...
void GraphicsView::SetRuleBarVisiable(bool value)
{
...
void GraphicsView::ZoomInUnderViewCenter()
{
...
void GraphicsView::ZoomOutUnderViewCenter()
{
    auto pos = QPoint(viewport()->rect().width()/2, viewport()->height()/2);
...
void GraphicsView::Zoom1To1(int x, int y, int w, int h)
{
    QRectF unity = transform().mapRect(QRectF(0, 0, 1, 1));
    if (unity.isEmpty())
        return;
...
void GraphicsView::ZoomToRect()
{

}

void GraphicsView::SetPan(bool value)
{
    d_ptr->mIsPan = value;
}
...
#if 0
    qDebug() << "GraphicsView::paintEvent";
...
void GraphicsView::Pan(QPointF delta)
{
    // 根据当前 zoom 缩放平移数
    delta *= d_ptr->mScale;
...
void GraphicsView::SetScale(double value) {
//    auto pos = QPoint(viewport()->rect().width()/2, viewport()->height()/2);
...
void GraphicsViewPrivate::SetRuleBarVisiable(bool value)
{
